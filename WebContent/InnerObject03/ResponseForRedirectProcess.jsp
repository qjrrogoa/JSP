...
<%
	...
	*/
	...
	//2]회원여부 판단(데이터 베이스의 테이블에서 조회)

	//3] 회원인경우 로그인 처리(session객체로 )후 마이페이지로 이동, 회원이 아니면 다시 로그인 페이지로 이동.
	if("KIM".equals(id) && "1234".equals(pwd)){ //회원인 경우
		...
		//방법2]쟈스로 메시지 띄운 후 이동(버퍼때문에 sendRedirect쓰면 안된다,)
		//out.println("<script>");
	...
	<%
		...
		//response.sendRedirect("ResponseForRedirectIndex.jsp"); == location.replace랑 똑같다.
		out.println("<script>");
