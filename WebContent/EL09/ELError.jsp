...
<body>
	<fieldset>
		...
		<h2>배열이나 리스트계열 컬렉션의 인덱스를 벗어난 경우(에러 NO, 출력NO)</h2>
		<ul>
		...
		<c:set var="member" value='<%=new MemberDTO("KIM","1234","김길동",null,null) %>'></c:set>
		<ul>
			<li>없는 속성 접근[PropertyNotFoundException에러] : \${member.addr }</li>
			<li>속성은 존재, getter가 없는 경우[에러] : ${member.id}</li> 
			<li>EL변수명이 틀린 경우[에러 NO, 출력 NO] : ${membe.id}</li> 
			<!-- 지금은 안나지만MemberDTO 클래스에서 getter 주석처리하면 에러남! -->
		</ul>
		...
		존재하지 않은 속성이나 키값으로 접근시 null임으로 에러는 안나고 출력만 안됨
		 ...
		 <ul>
		 	<li>\${requestScope.myrequset }[정상] : ${requestScope.myrequset }</li>
		 	<li>\${requestScope.yourequset }[에러 NO, 출력 NO] : ${requestScope.yourequset }</li>
		 	<li>\${map.mymap }[정상] : ${map.mymap } 
		 	<li>\${map.youmap }[에러 NO, 출력 NO] : ${map.youmap } 
		 	<li>\${pageContext.noproperty}[서블릿으로 변환X] : \${pageContext.noproperty}</li>
		 </ul>
		 
		 <h2>숫자를 0으로 나눈 Infinity</h2>
		 \${100/0 } : ${100/0 }
