...
<body>
	<fieldset>
		...
		<!-- JSP의 pageContext내장객체와 같다.
			단, 자바빈 규칙으로 접근 가능한 메서드만 제공한다. 
		-->
		<h4>자바코드로 컨텍스트 루트(/JSPProj) 얻기</h4>
		<ul>
			<li>방법1:request 내장객체 - <%=request.getContextPath() %>
			<%
		...
		<%=((HttpServletRequest)pageContext.getRequest()).getSession().getMaxInactiveInterval() %> <br/>
		...
		<!-- JSP로 요청 헤더 정보를 얻어오려면 request.getHeader("user-agent") -->
		<h4>자바코드로 요청 헤더값 얻기</h4>
		<%=request.getHeader("user-agent") %>
		<h4>EL로 요청 헤더값 얻기</h4>
		<!-- 
			EL식으로 요청헤더명에 따른 헤더값을 출력할때는
			header.요청헤더명이나
			혹은 header["요청헤더명"]
				단, 키값에 해당하는 요청헤더명에 특수 문자가 들어가 있는 경우에는 무조건 []사용
		 -->
		\${header.user-agent } : ${header.user-agent }<br/>
		\${header["user-agent"] } : ${header["user-agent"] }<br/>
		\${pageContext.request.headerNames} : ${pageContext.request.headerNames}
		
		<h2>3.EL의 cookie 내장객체</h2>
		<!-- 
			EL에서는 값을 설정하거나 영역에 속성을 저장하지는 못한다.
		 ...
		 <!-- 
		  ...
		  <!-- 
		  		컨텍스트 초기화 파라미터를 얻어 올 수 있는 내장 객체
		   -->
		   <!-- 
		   		[Context초기화 파라미터]
		   		-Context초기화 파라미터는 ServeltContext의
		   		getInitParameter("파라미터명")메서드를 통해서 얻는다.
		   		-Context를 구성하는 모든 서블릿에서 공유할 수 있는 변수
		   		
		   		[Servlet초기화 파라미터]
		   		-Servlet초기화 파마미터는 ServletConfig의 getInitparameter("파라미터명")
		   		메서드를 통해서 얻는다.
		  ...
		  <ul>
		  	...
		  	<li>ORACLE_DRIAVER : <%=application.getInitParameter("ORACLE_DRIVER") %></li>
