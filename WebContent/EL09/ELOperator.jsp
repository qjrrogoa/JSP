...
<body>
	<fieldset>
		...
		<ul>
			...
			<!-- EL에서는 +는 숫자 연산에만 사용가능. 문자열 연결시 에러  -->
		...
		<ul>
			<!-- 크기 비교 연산잔는 compareTo()메서드로 인식 -->
			<li>\${fnum>snum } : ${fnum>snum }</li> <!-- false -->
			<li>\${"100">"9" } : ${"100">"9" }</li> <!-- false -->
			<li>\${100>9 } : ${100>9 }</li> <!-- true -->
			<!-- 
				자바에서는 문자열 비교시 equals()메서드로
				비교하나 EL에서는 ==로비교
			 -->
		...
		<!-- 
		 ...
		 <ul>
		 	...
		 	<li>\${empty zeroLength} : ${empty zeroLength ? "크기가 0인 배열이다" :"크기가 0이 아닌 배열이다."}</li>
		 	<li>\${!empty zeroSize} : ${not empty zeroSize ? "객체가 저장되어 있다" : "객체가 저장되어 있지 않다"}</li>
		 </ul>
