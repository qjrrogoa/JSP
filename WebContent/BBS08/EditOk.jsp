...
<%
	...
	/*
	searchWord를 한글로 검색 후 수정시 주의 사항
	searchRedirect()메서드로 쿼리스트링 연결시 한글 깨짐
	해결책:
	1. 검색어를 URLEncoder.encode()
		단, 검색후 총 레코드 수 구할때는 인코딩하지 않는 검색어 전달
	2. 수정 완료 후 뷰로 이동시 뷰(View.jsp)에서는 URLDecoder.decode()해야 한다.
	
	단, form, 태그로 이동시에는 URL 인코딩 및 디코딩 불 필요
	*/
